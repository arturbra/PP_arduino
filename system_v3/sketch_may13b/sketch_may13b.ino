...
void createTemperatureFile() {
  ...
  if (!myFile) {
    ...
    if (myFile) {
      ...
      // The file failed to open in write mode, so print an error message
      Serial.println("Error creating temperature.txt file");
    }
  } else {
    // The file already exists
    Serial.println("Temperature file already exists");
    myFile.close(); // Close the file since we're done with it
  }
}
