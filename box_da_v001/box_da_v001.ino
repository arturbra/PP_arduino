...
void setup() {
  ...
  createTemperatureFile(8);
  createTemperatureFile(7);
  createRainfallFile();
...
void createTemperatureFile(int sensorNumber){
  String filename = "/box_da_temperature_" + String(sensorNumber) + ".txt";
...
void createRainfallFile(){
  myFile = SD.open("/box_da_rainfall.txt");
  if (!myFile) {
...
void saveRainfallValue(){
  myFile = SD.open("/box_da_rainfall.txt", FILE_APPEND);
  if (myFile) {
